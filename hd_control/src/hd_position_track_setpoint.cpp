...
int main(int argc, char **argv)
{
    ...
    track_enabled_pub = nh.advertise<std_msgs::Bool>("hd/position_track/position_track_enable", 10);
    ...
    gps_sub = nh.subscribe("dji_sdk/gps_position", 10, &gps_callback);
    ...
    tag_36h11_6->setToLandingCenterTranslation(Eigen::Vector3d(0.0, 0.0, 0.0));
    ...
    while (ros::ok())
    {
        ...
        if (found_16h5 || found_36h11)
        {
            ...
            track_enable_msg.data = true;
            track_enabled_pub.publish(track_enable_msg);
            ...
            else //found_16h5 not found
            {
                ...
                {
                    ROS_DEBUG_THROTTLE(10, "Ready to descend. " );
                    descend();
                }
                else
                {
